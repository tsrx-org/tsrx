...
import { honoTarget, resolveHonoTarget } from '../src/target.js';
...
describe('@tsrx/hono server compiler', () => {
	it('publishes one closed server-default target descriptor', () => {
		expect(honoTarget.defaultMode).toBe('server');
		expect(resolveHonoTarget()).toEqual({
			compiler: '@tsrx/hono',
			jsxImportSource: 'hono/jsx',
		});
		expect(resolveHonoTarget('dom')).toEqual({
			compiler: '@tsrx/hono/dom',
			jsxImportSource: 'hono/jsx/dom',
		});
		expect(() => resolveHonoTarget(/** @type {any} */ ('client'))).toThrow(/Invalid Hono/);
	});
...
describe('@tsrx/hono DOM compiler', () => {
	...
	it('reports each rendered local async component once', () => {
		const result = compileDomToVolarMappings(
	...
	it('reports each rendered TSRX statement-container async component once', () => {
		const result = compileDomToVolarMappings(
			`async function First(items) @{
				for await (const item of items) {
					console.log(item);
	...
	it('adds a mapped type-only assertion for unresolved component return types', () => {
		const source = `import { Remote } from './remote.js';
			export function App() @{ <Remote /> }`;
		const result = compileDomToVolarMappings(source, 'App.tsrx');

		expect(result.errors).toEqual([]);
		expect(result.code).toContain('__tsrx_hono_dom_error');
		expect(result.code).toContain('typeof Remote');
		expect(result.code).toContain('(Remote)');
		expect(
			result.mappings.some(
				(mapping) =>
					mapping.data.verification === true &&
					mapping.sourceOffsets.some(
						(offset, index) =>
							offset <= source.lastIndexOf('Remote') &&
							offset + mapping.lengths[index] >= source.lastIndexOf('Remote') + 'Remote'.length,
					),
			),
		).toBe(true);
	});

	it('keeps the Promise assertion out of runtime output', () => {
		const { code } = compileDom(
		...
		expect(code).not.toContain('__tsrx_hono_dom_error');
	});
