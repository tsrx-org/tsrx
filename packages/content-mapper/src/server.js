#!/usr/bin/env node
/**
 * Content-mapper process entry: speaks `Content-Length`-framed JSON-RPC 2.0
 * over stdio. TypeScript is the only requester; the mapper never sends
 * requests or notifications. stdout is reserved for protocol messages, so
 * everything else (including `console.log`) goes to stderr.
 */
...
/**
 * Run the mapper server on the given streams.
