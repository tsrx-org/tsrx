...
describe('@tsrx/vite-plugin-solid web workers', () => {
	...
	it('compiles a dev worker entry from its file path', async () => {
		const dir = mkdtempSync(join(tmpdir(), 'tsrx-solid-worker-'));
		const real_path = join(dir, 'Worker.tsrx');
		writeFileSync(real_path, worker('hello'));

		const result = await call_load(tsrxSolid(), real_path + '.tsx?worker_file&type=module');
	...
	it('invalidates a dev worker entry on hot update', async () => {
		...
		const modules = await call_handle_hot_update(tsrxSolid(), {
